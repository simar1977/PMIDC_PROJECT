...
input::-webkit-inner-spin-button {
  ...
  margin: 0;
		...
		<div class="form-group  required " id="dd"> 
									 		...
									 		<div class="col-lg-8">
                                       <select class="form-control" id='district' name="districtBean" onchange="loadUlb()"></select>
									 		</div>
									 	</div>
		...
		<div class="form-group required">
										...
										<div class="col-lg-8">
										<input class="form-control" type="number" name="tokenNo"  id="tokenNo"   pattern="[a-zA-Z0-9/-]+" title="only numbers are allowed">
										</div>
									</div>
					...
					<table class="position-center" id="elist" style="table-layout: fixed;width:100%;word-wrap:break-word;border: 1px solid black;">
						<thead>
						  <tr style="font-size:11px">
							...
							<th width="4%" >Name of Applicant </th>
							<th width="3%">Father/Spouse Name</th>
							<th width="4%">District </th>
							<th width="4%">ULB</th>
							<th width="4%">Locality </th>
							<th width="5%">Mobile Number</th>
							<th width="6%">Permanent Address</th>
							<th width=6%>Date Of Application</th>
		   ...
		   if(item.status=="Active")
		    	{
		    		...
		    		var p=p2+"-"+p3+"-"+p1;	
		    		var sp=s3+"-"+s2+"-"+s1;
		    		if(item.status=="Active" && item.fee=="Yes" && item.applicationStatus=="Approved")
		    		{
		    		if(p1==s1-1)
		    			{
		    			alert("As Your Application  "+item.petId +"  Is already Expried So Please Renew It !");
		    			
		    			}
		    		htm=htm+"<td> <select id=\"actionDD"+item.petId+"\" name=\"actionDD"+item.petId+"\" onChange=\"getaction("+item.petId+")\"><option value=\"\">Select from below</option><option value=\"Renew Pet\"><button style=\"color:teal; background-color: #FFFFC0; border: 10px ridge lightgrey\">Download PET LICENSE</button></option>";
		    		
		    		}
		    	}
		 
		   else if(item.status!="Active" || p1-1==s1 || item.fee=="No")
			   htm=htm+"<td> <select id=\"actionDD"+item.petId+"\" name=\"actionDD"+item.petId+"\" onChange=\"getaction("+item.petId+")\"><option value=\"\" >Download option is Not Available !</option>";
		   	htm=htm+"</select></td>"+		    		
    		 	"</tr>";
	    		...
	    		{
	    				window.location="/pet-license/downloads-pet-license?petId="+id;
	    		}
