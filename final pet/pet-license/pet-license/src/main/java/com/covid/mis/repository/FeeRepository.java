...
public interface FeeRepository extends CrudRepository<FeeBean, Integer>{
	...
	@Query("FROM FeeBean where headBean.districtBean.districtCode=:districtCode and status='paid'")
	...
	@Query("FROM FeeBean where orderId=:string")
	FeeBean getOrderId(@Param("string") String string);

	@Query("FROM FeeBean where transDate< :date and status='created'")
	List<FeeBean> getPendingOrders(@Param("date") Date date);
